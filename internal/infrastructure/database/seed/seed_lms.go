...
func seedLMS(db *gorm.DB, ctx context.Context, state *State) error {
	...
	courseID, classID := state.Classes[0].CourseID, state.Classes[0].ID
	...
	enrollments := []database.CourseEnrollment{{ID: enrollmentID, CourseID: courseID, StudentID: studentID, TeacherNote: "Enrolled for 2026/2027."}}
	...
	}{{"course_teachers", map[string]any{"course_id": courseID, "user_id": teacherID}}, {"assignment_teachers", map[string]any{"assignment_id": assignmentID, "user_id": teacherID}}, {"semester_transcript_enrollments", map[string]any{"semester_transcript_id": semesterID, "course_enrollment_id": enrollmentID}}} {
